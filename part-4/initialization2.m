function result = initialization2()
    addpath('..');
    addpath('Road');
    ...
    img=double(imread('test0000.jpg'));
    [H,W,~]=size(img);
    N=H*W;

    cameras=fopen('cameras.txt','r');
    ...
    for n=nei_num:140-nei_num
        img_cen=double(imread(['test',sprintf('%04d',n),'.jpg']));
        ...
        for b=[n-3,n-2,n-1,n+1,n+2,n+3]
            img_nei=double(imread(['test',sprintf('%04d',b),'.jpg']));
            seq=b*7;
